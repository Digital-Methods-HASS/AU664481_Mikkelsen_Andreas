...
author: "Andreas E. Mikkelsen og Lea Skriver Hansen"
...
```{r}
#second try, first one gave a single graph and not separate 
ggplot(data = New_Western_Europe) + 
  geom_line(mapping = aes(x = Year, 
...
konger <- read_csv2("./data/KongerækkeDK.csv",na = c("NA","N/A"))
```
```{r}
  konger2 <- konger %>% 
  mutate(midyear = konger$Reign_End - (konger$Reign_End-konger$Reign_Start)/2)  # creating midyear column
```

```{r}
ggplot(konger2, aes(x = midyear, y = konger2$Reign_Total_Year)) +
  geom_point()+    
  geom_smooth()+   
...
## task 7
is hard to comment anything only by this, there are a fair few facts this does bot show us. If we only focus on this data, it would seem like we have stopped killing each other as much and the kings somewhat rule longer than before, but not by much.
